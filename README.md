...
## Final Results
he AlexNet model held the least amount of overfitting of the models and highest accuracy, just 3% better than the ResNet model, for this dataset. Overfitting was a big issue with this dataset and was a difficult to overcome. I did not expect the AlexNet model to reach the 91% accuracy with low overfitting. Batch normalization in each hidden layer and dropouts of .1 in every other hidden layers helped remove overfitting. The 70/30 split of training and test data also helped because I tried 90/10 initially due to the small dataset, then 80/20 and could not overcome the overfitting problem. In addition, using ReduceLROnPlateau function helped train the model by controlling the adjustments on weights on different epochs as needed (if validation accuracy was not increasing after 1 epoch). I started with a low learning rate of .0001, but due to the overfitting of past models, allowed the model to decrease to .00000000001 if needed. Forcing the model to slow down helped training by not passing local minimums and this model decreased three times during training, increasing and maintain accuracy without overfitting.
